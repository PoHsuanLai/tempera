...
/// Spawn a styled switch. Returns the root entity. Observe
/// `ValueChange<bool>` on it to react to flips.
pub fn spawn_switch(commands: &mut Commands, style: &SwitchStyle, checked: bool) -> Entity {
    ...
    commands.spawn((
        ...
        Node {
            ...
            top: Val::Px(inset),
            left: Val::Px(if checked {
                size.track_width() - size.thumb_diameter() - inset
            } else {
                inset
            }),

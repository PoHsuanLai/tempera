...
use bevy::prelude::*;
use tempera_dock::registry::Panes;
use tempera_dock::{
    ActivePage, Axis, DockCommands, DockLayout, DockTree, Page, PageId, PaneRegistry,
    PaneVisibility, Side, TemperaDockPlugin, page_is_active, pane_exists,
};
...
fn pages_work_with_no_dock_present() {
    ...
    // No `TemperaDockPlugin` at all — just the one system, which is the point:
    // a page's crate must be testable without standing up a layout.
    let mut app = App::new();
    app.add_systems(Update, tempera_dock::page::apply_active_page);
    ...
    assert!(
        app.world().get::<tempera_dock::Pane>(container).is_none(),

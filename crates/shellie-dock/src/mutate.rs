...
impl DockCommands<'_> {
    ...
    pub fn split_pane(
        ...
        if self.layout.root.find_pane(new_pane.as_str()).is_some() {
            warn!(
                "[shellie-dock] split_pane: {:?} already exists; ignoring",
                new_pane.as_str()
            );
            return;
        }
        if self.layout.root.find_pane(target).is_none() {
            warn!("[shellie-dock] split_pane: no pane {target:?}; ignoring");
            return;
        }
    ...
    pub fn remove_pane(&mut self, target: &str) {
        if self.layout.root.find_pane(target).is_none() {
            warn!("[shellie-dock] remove_pane: no pane {target:?}; ignoring");
            return;
        }
        if self.layout.pane_ids().count() <= 1 {
            warn!("[shellie-dock] remove_pane: {target:?} is the last pane; ignoring");
            return;
        }
    ...
    pub fn move_pane(&mut self, target: &str, into: &str, axis: Axis, side: Side) {
        ...
        let Some(moved) = self.layout.root.find_pane(target).cloned() else {
            warn!("[shellie-dock] move_pane: no pane {target:?}; ignoring");
            return;
        };
        if self.layout.root.find_pane(into).is_none() {
            warn!("[shellie-dock] move_pane: no pane {into:?}; ignoring");
            return;
        }
        if !remove_pane_from(&mut self.layout.root, target) {
            return;

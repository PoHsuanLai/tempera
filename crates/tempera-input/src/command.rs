...
//! the extension model. The components below are the ones tempera knows how
//! to interpret; an application is free to add its own to the same entity and
//! query them, which is how things tempera has no business modelling (a
//! palette grouping, an icon, an owning extension id) get attached without
//! tempera growing a field for them.
//!
//! ```
//! use bevy::prelude::*;
//! use tempera_input::chord::cmd as chord;
//! use tempera_input::command::{AppCommandExt, Command, CommandLabel, Keybind, cmd, on_press};
...
//! tempera_input::fire::<Undo>(app.world_mut());
...
///
/// The concrete type is the application's and is never named by tempera. The
...
/// monomorphized together — so it would mean tempera itself mismatched a
...
pub fn on_release<T: Send + Sync + 'static>(
    ...
    OnRelease(Arc::new(move |world, payload| {
        match payload.downcast::<T>() {
            ...
            Err(_) => {
                tracing::error!(
                    "[tempera-input] held payload type mismatch on release; \
...
/// `command id -> entity`, maintained by tempera.
///
...
impl CommandRegistry {
    ...
    fn insert(&mut self, id: String, entity: Entity) -> bool {
        if let Some(existing) = self.by_id.get(&id) {
            error!(
                "[tempera-input] duplicate command id {id:?}: already registered \
...
impl AppCommandExt for App {
    fn spawn_command(&mut self, bundle: impl Bundle) -> &mut Self {
        ...
        let Some(id) = world.get::<CommandId>(entity).map(|c| c.0.clone()) else {
            error!(
                "[tempera-input] spawn_command called with a bundle carrying no \
...
pub fn fire_by_id(world: &mut World, id: &str) -> bool {
    let Some(entity) = world
        ...
        warn!("[tempera-input] fire: no command registered as {id:?}");
        return false;
    };

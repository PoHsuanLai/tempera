...
use bevy::prelude::*;
use shellie_input::binding::{Binding, resolve};
use shellie_input::chord::{cmd, key};
use shellie_input::command::{
    AppCommandExt, BindScope, Command, CommandLabel, CommandRegistry, Keybind, on_press,
};
use shellie_input::persist::SavedKeybinds;
use shellie_input::plugin::ShellieInputPlugin;
use shellie_input::{Chord, rebind};
...
fn test_app() -> App {
    let mut app = App::new();
    app.add_plugins(ShellieInputPlugin::new("shellie-e2e-test-unused"));
...
fn register_save(app: &mut App) {
    app.spawn_command(shellie_input::cmd::<Save>((
...
fn unbinding_leaves_the_command_fireable_by_id_only() {
    ...
    assert!(shellie_input::fire::<Save>(app.world_mut()));
    assert_eq!(app.world().resource::<Fired>().0, vec!["save"]);
}

...
//!
//! shellie does not know it is running dawai. [`SavedKeybinds::load`] takes an
//! app name and derives `<config dir>/<app>/keybinds.json` from it, so two
//! applications built on shellie do not fight over one file.
...
mod tests {
    ...
    fn storage_path_is_namespaced_by_app() {
        ...
        assert_ne!(a, b, "two shellie apps must not share a keybinds file");

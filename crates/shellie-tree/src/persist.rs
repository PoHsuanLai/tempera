...
//!
//! `shellie-input` treats an unknown saved keybind as ignorable rather than as
//! a corrupt file, and `shellie-dock` keeps a saved pane the build no longer
...
impl TreeState {
    ...
    pub fn apply(&self, world: &mut World, id: &TreeId) {
        if self.version > FORMAT_VERSION {
            error!(
                "[shellie-tree] saved state for {:?} is format {} but this build understands \
        ...
        if targets.is_empty() {
            warn!(
                "[shellie-tree] no tree view carries the id {:?}; the saved expansion was not \
                 applied",
                id.as_str(),
            );
            return;

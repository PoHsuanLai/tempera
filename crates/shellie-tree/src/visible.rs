...
pub fn visible_rows(nodes: &[TreeNode<'_>], state: &TreeState, query: &str) -> Vec<VisibleRow> {
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        if let Some(&first) = node_of_group.get(group) {
            warn!(
                "[shellie-tree] two items claim the group id {:?} ({:?} and {:?}); \
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        match node.parent {
            ...
            Some(parent) => {
                warn!(
                    "[shellie-tree] {:?} names parent group {:?}, which no item declares; \
                     dropping the row",
                    node.entity,
                    parent.as_str(),
                );
    ...
    while let Some((index, depth)) = stack.pop() {
        ...
        if !open_path.insert(group) {
            warn!(
                "[shellie-tree] group {:?} is its own ancestor; stopping the walk there",
                group.as_str(),
            );
            continue;
        }
